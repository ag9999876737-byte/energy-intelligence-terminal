...
RSS_FEEDS = [
    # High-signal institutional sources
    ...
    ("AFR Business",            "https://www.afr.com/rss"),
...
def render_card(d):
    ...
    st.markdown(f"""
    <div class="card">
        <span class="tag {status_cls}">{d['status']}</span>
        <span class="tag {type_cls}">{d['type']}</span>
        <span class="tag tag-sector">{d['sector']}</span>
        {country_tags}
        ...
        {summary_block}
        <a class="read-more" href="{d['link']}" target="_blank">🔗 Read full article →</a>
    </div>
    """, unsafe_allow_html=True)
...
st.markdown("---")
st.caption(
    "Sources: DealStreet Asia · Business Wire · GlobeNewswire · PR Newswire · "
    "Reuters · AFR · RenewEconomy · Nikkei Asia  ·  "
